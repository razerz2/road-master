                    ...
                    <form method="POST" action="{{ route('vehicles.update', $vehicle) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="year" class="block mt-1 w-full" type="number" name="year" :value="old('year', $vehicle->year)" />
                            ...
                            <div>
                                ...
                                <x-text-input id="tank_capacity" class="block mt-1 w-full" type="number" step="0.01" name="tank_capacity" :value="old('tank_capacity', $vehicle->tank_capacity)" />
                                <x-input-error :messages="$errors->get('tank_capacity')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="current_odometer" :value="__('Odômetro Atual (km)')" />
                                <x-text-input id="current_odometer" class="block mt-1 w-full" type="number" name="current_odometer" :value="old('current_odometer', $vehicle->current_odometer)" />
                                <x-input-error :messages="$errors->get('current_odometer')" class="mt-2" />
                            </div>

                            <div>
                                <label class="flex items-center">
                                    <input type="checkbox" name="active" value="1" {{ old('active', $vehicle->active) ? 'checked' : '' }} class="rounded border-gray-300">

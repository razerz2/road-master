    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __('Gerenciar Métodos de Pagamento') }}
            </h2>
                    ...
                    <div x-data="{ editingId: @js($editingId ?? null) }">
                        ...
                        <div class="mb-6 p-4 bg-gray-50 dark:bg-gray-900 rounded-lg">
                            <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">
                                <span x-show="!editingId">Novo Método de Pagamento</span>
                                <span x-show="editingId" x-cloak>Editar Método de Pagamento</span>
                            </h3>
                            ...
                            @if(isset($paymentMethod))
                                <form method="POST" action="{{ route('payment-methods.update', $paymentMethod) }}">
                                    @csrf
                                    @method('PUT')
                                    @include('payment-methods.form', ['paymentMethod' => $paymentMethod])
                                </form>
                            @else
                                    ...
                                    @forelse($paymentMethods as $pm)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <a 
                                                    href="{{ route('payment-methods.index', ['edit' => $pm->id]) }}"

                                    ...
                                    <tr class="location-row hover:bg-gray-50 dark:hover:bg-gray-700/50 transition" 
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            ...
                                            <form action="{{ route('locations.destroy', $location) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-red-600 hover:text-red-900" onclick="event.preventDefault(); const form = this.closest('form'); if (typeof handleDelete === 'function') { handleDelete(form, 'Tem certeza que deseja excluir este local?'); } else { if (confirm('Tem certeza que deseja excluir este local?')) { form.submit(); } }" title="Excluir">
                                                    <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">

                    ...
                    <form method="POST" action="{{ route('trips.update', $trip) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select id="vehicle_id" name="vehicle_id" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm" required>
                                    @foreach($vehicles as $vehicle)
                                        <option value="{{ $vehicle->id }}" {{ old('vehicle_id', $trip->vehicle_id) == $vehicle->id ? 'selected' : '' }}>{{ $vehicle->name }} - {{ $vehicle->plate }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <div class="flex items-end gap-2">
                                    <div class="flex-1">
                                        ...
                                        <input type="hidden" id="driver_id_hidden" name="driver_id" value="{{ old('driver_id', $trip->driver_id) }}">
                            ...
                            <div>
                                ...
                                <x-text-input id="date" class="block mt-1 w-full" type="date" name="date" :value="old('date', $trip->date->format('Y-m-d'))" required />
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <select id="origin_location_id" name="origin_location_id" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm" required>
                                            ...
                                            @foreach($locations as $location)
                                                <option value="{{ $location->id }}" {{ old('origin_location_id', $trip->origin_location_id) == $location->id ? 'selected' : '' }}>{{ $location->name }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="flex items-end gap-2">
                                    <div class="flex-1">
                                        ...
                                        <select id="destination_location_id" name="destination_location_id" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm" required>
                                            <option value="">Selecione...</option>
                                            @foreach($locations as $location)
                                                <option value="{{ $location->id }}" {{ old('destination_location_id', $trip->destination_location_id) == $location->id ? 'selected' : '' }}>{{ $location->name }}</option>
                                            @endforeach
                                        </select>
                            ...
                            <div>
                                <label class="flex items-center mt-6">
                                    <input type="checkbox" name="return_to_origin" value="1" {{ old('return_to_origin', $trip->return_to_origin) ? 'checked' : '' }} class="rounded border-gray-300">
                            ...
                            <div>
                                ...
                                <x-text-input id="departure_time" class="block mt-1 w-full" type="time" name="departure_time" :value="old('departure_time', $trip->departure_time)" required />
                                <x-input-error :messages="$errors->get('departure_time')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="return_time" :value="__('Horário de Retorno')" />
                                <x-text-input id="return_time" class="block mt-1 w-full" type="time" name="return_time" :value="old('return_time', $trip->return_time)" />
                            ...
                            <div>
                                ...
                                <x-text-input id="odometer_end" class="block mt-1 w-full" type="number" name="odometer_end" :value="old('odometer_end', $trip->odometer_end)" required />
                                <x-input-error :messages="$errors->get('odometer_end')" class="mt-2" />
                            </div>

                            <div class="md:col-span-2">
                                <x-input-label for="purpose" :value="__('Finalidade / Observações')" />
                                <textarea id="purpose" name="purpose" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm">{{ old('purpose', $trip->purpose) }}</textarea>
                        ...
                        <div class="mt-8 border-t pt-6">
                            ...
                            <div id="stops-container" class="space-y-4">
                                ...
                                <div class="border rounded-lg p-4 bg-gray-50 dark:bg-gray-700" id="stop-{{ $index }}">
                                    <div class="flex items-center gap-2 mb-3">
                                        <svg class="w-5 h-5 text-gray-400 cursor-move" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 8h16M4 16h16"></path>
                                        </svg>
                                        <h4 class="font-medium text-gray-800 dark:text-gray-200 flex-1">Parada <span class="stop-number">{{ $index + 1 }}</span></h4>
                                        <button type="button" onclick="removeStop({{ $index }})" class="text-red-600 hover:text-red-800 text-sm">
                                            Remover
                                        </button>
                                    </div>
                                    <div>
                                        <div class="flex items-end gap-2">
                                            <div class="flex-1">
                                                ...
                                                <select name="stops[{{ $index }}][location_id]" id="stop-{{ $index }}-location" class="block w-full rounded-md border-gray-300 shadow-sm" required>
                                                    <option value="">Selecione...</option>
                                                    @foreach($locations as $location)
                                                        <option value="{{ $location->id }}" {{ $stop->location_id == $location->id ? 'selected' : '' }}>{{ $location->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <button type="button" onclick="openLocationModal('stop-{{ $index }}-location')" class="px-3 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 transition text-sm whitespace-nowrap mb-1" title="Cadastrar novo local">
                                                + Novo
                                            </button>
                                        </div>
                                    </div>
        ...
        function addStop() {
            ...
            stopDiv.innerHTML = `
                <div class="flex items-center justify-between mb-3">
                    <h4 class="font-medium text-gray-800 dark:text-gray-200">Parada ${stopIndex + 1}</h4>
